...
public class UserCreatedRabbitMQConsumer : BackgroundService
{
    private readonly IConfiguration _configuration;
    private readonly ILogger<UserCreatedRabbitMQConsumer> _logger;
    private readonly IServiceProvider _serviceProvider;
    private IConnection _connection;
    private IChannel _channel;

    public UserCreatedRabbitMQConsumer(
        IConfiguration configuration,
        ILogger<UserCreatedRabbitMQConsumer> logger,
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            var queueName = "user-created-queue-notifications";
            ...
            consumer.ReceivedAsync += async (model, ea) =>
            {
                try
                {
                    ...
                    if (userEvent != null)
                    {
                        ...
                        var userConsumer = scope.ServiceProvider.GetRequiredService<UserCreatedConsumer>();

                        await userConsumer.ProcessAsync(userEvent);
